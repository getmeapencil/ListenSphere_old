...
function FormInput({label, type, name, inputs, handleChange, bio=false}) {
  return (
    <>
      ...
      {
        (bio === false) ? 
        <input required type={type} name={name} defaultValue={inputs[name] || ""} onChange={handleChange} className={`w-96 h-16 px-[0.5rem] py-[0.25rem] border-[3px] border-black rounded-2xl hover:ring hover:ring-label-blue focus:ring focus:ring-label-blue items-center ml-auto`} /> : 
        (<textarea required type={type} name={name} defaultValue={inputs[name] || ""} onChange={handleChange} className={`w-96 h-48 px-[0.5rem] py-[1rem] border-[3px] border-black rounded-2xl hover:ring hover:ring-label-blue focus:ring focus:ring-label-blue items-center ml-auto`}></textarea>)
      }
    </>
  )
}
...
function Form() {
  ...
  return (
    <form onSubmit={handleSubmit} className="translate-y-16 flex flex-col items-center border-4 border-black px-4 pt-4 pb-2 rounded-2xl bg-gradient-to-r from-card-grad-l to-card-grad-r "> 
